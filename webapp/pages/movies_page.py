...
def _scan_local_movie_file(app_state, imdb_id):
    ...
    try:
        for dirpath, _, filenames in os.walk(root):
            if needle not in dirpath.lower():
                continue
...
async def render(app_state, client: Client, imdb_id: str):
    ...
    release_date_display = format_date_display(release_date)

    plex_path = _get_movie_plex_file_path(library_movie) if library_movie is not None else ""
    local_path = await asyncio.get_event_loop().run_in_executor(None, _scan_local_movie_file, app_state, imdb_id)
    path_to_show = _display_relative_path(app_state, plex_path or local_path)

    in_plex = bool(plex_path)
    ...
    def _render_content_info():
        ...
        with ui.row().classes("w-full items-center no-wrap px-2 py-2").style(
            ...
            ui.label("Relative path").style("width: 50%")
            ui.label("Release version category").style("width: 16%")
            ui.label("Status").style("width: 25%")
        ...
        with ui.row().classes("w-full items-center no-wrap px-2 py-2").style(
            ...
            ui.label(path_to_show).style(
                f"width: 50%; color:{COLORS['text_muted']}; white-space:nowrap; overflow:hidden; text-overflow:ellipsis"
            )
            ui.label(category).style("width: 16%")

            with ui.row().classes("items-center gap-1").style("width: 25%"):
